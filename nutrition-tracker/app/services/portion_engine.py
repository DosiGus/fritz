...
from app.services.static_data import VOLUME_DENSITY, get_portion_rule
...
def resolve_portion(
    ...
    """Resolve a parsed food item into grams/ml or a clarification request.

    Priority:
    1. Explicit g/ml/kg/l quantities → deterministic conversion.
    2. Personal memory ("eine Tasse Kaffee" = 200ml for this user).
    3. Portion rules from DB (or static fallback).
    4. LLM-supplied options carried through as needs_clarification.
    5. Generic Klein/Normal/Groß clarification.
    """
    ...
    if item.needs_clarification and item.options:
        ...
        _audit(db, user_id, "portion_clarification_needed", item, carried, "llm_options")
        return carried

    rule = _lookup_rule(item.name, unit, db)
    if rule:
        resolved = _resolve_from_rule(item, quantity, rule)
...
def _lookup_rule(food_name: str, unit: str, db: Session | None) -> Any | None:
    if db is not None:
        rule = PortionRuleRepository(db).get_by_food_and_unit(food_name, unit)
        if rule:
            return rule
